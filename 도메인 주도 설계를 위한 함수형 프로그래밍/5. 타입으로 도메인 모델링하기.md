...
### 불변성과 정체성
- 함수형 프로그래밍에서 값 객체는 기본적으로 불변으로 생성된 값을 변경할 수 없음
- 하지만 엔티티는 시간이 지남에 따라 데이터가 변할것으로 기대함
- 이것이 바로 엔티티에 식별자가 있는 이유이며 변경시 복사본을 만들면서 식별자를 유지하면됨
- 불변 데이터를 사용하면 모든 변경 명세가 함수 시그니처에 명시적으로 드러나는 이점이 있음
...
class Person {
  [immerable] = true;
  constructor(readonly id: PersonId, readonly name: string) {}
}

const initialPerson = new Person(new PersonId("123"), "John Doe");

const updatedPerson = produce(initialPerson, (draft) => {
  draft.name = "Dongwoo";
});

console.log(initialPerson.name); // John Doe
console.log(updatedPerson.name); // Dongwoo
```
