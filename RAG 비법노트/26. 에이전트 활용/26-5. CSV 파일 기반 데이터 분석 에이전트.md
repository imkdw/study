...
with st.sidebar:
    ...
    selected_model = st.selectbox(
        "OpenAI 모델을 선택해주세요.", ["gpt-4o-mini", "gpt-3.5-turbo"], index=0
    )  # OpenAI 모델 선택 옵션
...
if apply_btn and uploaded_file:
    try:
        # 여러 파일이 업로드된 경우 첫 번째 파일만 사용
        if isinstance(uploaded_file, list):
            if len(uploaded_file) > 0:
                file_to_load = uploaded_file[0]
                if len(uploaded_file) > 1:
                    st.warning(
                        f"{len(uploaded_file)}개의 파일이 업로드되었습니다. 첫 번째 파일 '{file_to_load.name}'만 사용합니다."
                    )
            else:
        ...
        loaded_data = pd.read_csv(file_to_load)  # CSV 파일 로드
        ...
        # 데이터 프리뷰 표시
        st.success(f"'{file_to_load.name}' 파일이 성공적으로 로드되었습니다!")
        st.info(f"데이터 형태: {loaded_data.shape[0]}행 {loaded_data.shape[1]}열")
